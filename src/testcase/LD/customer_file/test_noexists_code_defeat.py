...
class TestImportProductCodeExists(BaseTest):
    """
    用例名：noexists_code_defeat
    用例ms的id：101231
    """

    def test_noexists_code_defeat(self, page: Page, project_name: str):
        f"""产品编码不存在导入失败验证（客户产品映射页面）
        用例名：noexists_code_defeat
        用例ms的id：101231
        项目名：{project_name}
        ...
        """
        logger.info("=" * 60)
        logger.info(f"开始执行测试用例：noexists_code_defeat")
        logger.info(f"用例ms的id：101231")
        logger.info("=" * 60)

        # 定义测试文件路径
        excel_file = r"D:\install\auto_ui-master\src\data\LD\customer_file\101231测试文件.xlsx"
        ...
        logger.info(f"测试用例noexists_code_defeat执行完成")