...
class TestImportProductCodeExists(BaseTest):
    """
    用例名：noexists_code_create
    用例ms的id：101237
    """

    def test_noexists_code_create(self, page: Page, project_name: str):
        f"""测试导入客户存在且新产品编码的场景
        用例名：noexists_code_create
        用例ms的id：101237
        项目名：{project_name}
        ...
        """
        logger.info("=" * 60)
        logger.info(f"开始执行测试用例：noexists_code_create")
        logger.info(f"用例ms的id：101237")
        logger.info("=" * 60)

        # 定义测试文件路径
        excel_file = r"D:\install\auto_ui-master\src\data\LD\customer_file\101237测试文件.xlsx"
        ...
        logger.info(f"测试用例noexists_code_create执行完成")