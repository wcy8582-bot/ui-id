...
class TestImportProductCodeExists(BaseTest):
    """
    用例名：mismatch_productname
    用例ms的id：101268
    """

    def test_mismatch_productname(self, page: Page, project_name: str):
        f"""测试导入不匹配产品名称场景
        用例名：mismatch_productname
        用例ms的id：101268
        项目名：{project_name}
        ...
        """
        logger.info("=" * 60)
        logger.info(f"开始执行测试用例：mismatch_productname")
        logger.info(f"用例ms的id：101268")
        logger.info("=" * 60)

        # 定义测试文件路径
        excel_file = r"D:\install\auto_ui-master\src\data\LD\customer_file\101268测试文件.xlsx"
        ...
        logger.info(f"测试用例mismatch_productname执行完成")