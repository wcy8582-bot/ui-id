...
class TestImportProductCodeExists(BaseTest):
    ...
    def test_import_product_code_exists(self, page: Page, project_name: str):
        f"""测试导入不存在产品编码场景
        用例名：import_product_code_noexists
        用例ms的id：101231
        项目名：{project_name}
        ...
        """
        logger.info("=" * 60)
        logger.info(f"开始执行测试用例：import_product_code_noexists")
        logger.info(f"用例ms的id：101246")
        logger.info("=" * 60)

        # 定义测试文件路径
        excel_file = r"D:\install\auto_ui-master\src\data\LD\customer_file\101246测试文件.xlsx"
        ...
        logger.info(f"测试用例import_product_code_noexists执行完成")