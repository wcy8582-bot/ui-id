...
class TestVerifyWorkOrderBatch(BaseTest):
    """
    用例名：verify_work_order_batch
    用例ms的id：101100
    """

    def test_verify_work_order_batch(self, page: Page, project_name: str):
        f"""测试生产工单批号编码规则查看流程
        用例名：verify_work_order_batch
        用例ms的id：101100
        项目名：{project_name}
        ...
        """
        logger.info("=" * 60)
        logger.info(f"开始执行用例verify_work_order_batch")
        logger.info(f"用例ms的id：101100")
        logger.info("=" * 60)
        ...
        for page_index in range(max_pages):
            logger.info(f"正在第 {page_index + 1} 页查找业务类型为'生产工单批号'的规则")

            # 在当前页查找目标行
            target_row = content_frame.locator("tr").filter(
                has=content_frame.locator("td").nth(3).get_by_text("生产工单批号")
            ).first
        ...
        if target_row is None or not target_row.is_visible():
            logger.error("未找到业务类型为'生产工单批号'的规则")
            raise AssertionError("未找到业务类型为'生产工单批号'的规则")

        # 验证找到目标行
        expect(target_row).to_be_visible()
        logger.info("成功定位到第一个业务类型为'生产工单批号'的规则")
        ...
        logger.info(f"用例verify_work_order_batch执行完成")