...
class TestVerifyCodingRuleConfig(BaseTest):
    """
    用例名：verify_purchase_inbound_initialization
    用例ms的id：101094
    """

    def test_verify_purchase_inbound_initialization(self, page: Page, project_name: str):
        f"""测试采购入库批号编码规则
        用例名：verify_purchase_inbound_initialization
        用例ms的id：101094
        项目名：{project_name}
        ...
        """
        logger.info("=" * 60)
        logger.info(f"开始执行verify_purchase_inbound_initialization")
        logger.info(f"用例ms的id：101094")
        logger.info("=" * 60)
        ...
        for page_index in range(max_pages):
            logger.info(f"正在第 {page_index + 1} 页查找业务类型为'采购入库批号'的规则")

            # 在当前页查找目标行
            target_row = content_frame.locator("tr").filter(
                has=content_frame.locator("td").nth(3).get_by_text("采购入库批号")
            ).first
        ...
        if target_row is None or not target_row.is_visible():
            logger.error("未找到业务类型为'采购入库批号'的规则")
            raise AssertionError("未找到业务类型为'采购入库批号'的规则")

        # 验证找到目标行
        expect(target_row).to_be_visible()
        logger.info("成功定位到第一个业务类型为'采购入库批号'的规则")
        ...
        logger.info("verify_purchase_inbound_initialization 用例执行完成")