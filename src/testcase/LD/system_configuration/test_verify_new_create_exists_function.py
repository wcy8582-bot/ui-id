...
class TestLabelPrintTemplateCreate(BaseTest):
    """
    用例名：verify_new_create_exists_function
    用例ms的id：101112
    """

    def test_verify_new_create_exists_function(self, page: Page, project_name: str):
        f"""验证新增标签重复模板ID
        用例名：verify_new_create_exists_function
        用例ms的id：101112
        项目名：{project_name}
        ...
        """
        logger.info("=" * 60)
        logger.info(f"开始执行用例：verify_new_create_exists_function")
        logger.info(f"用例ms的id：101112")
        logger.info("=" * 60)
        ...
        # 填写模板信息
        logger.info("填写模板基础信息，模板ID=666，模板名称=测试")
        frame.get_by_role("textbox", name="请输入模板ID").fill("666")
        ...
        logger.info(f"用例verify_new_create_exists_function执行完成")